...
| MCP server | `mcp_servers/dragonfruit_mock_server.py` | FastMCP (official MCP Python SDK) exposing 6 tools and 7 `dragonfruit://` resources over `mock_data/*.json`, stdio transport. |
...
**MCP path:** any MCP client launches `dragonfruit_mock_server.py` over stdio and queries the same `mock_data/` JSON through typed tools (`get_pipeline`, `get_time_allocations(role)`, …) or URI resources (`dragonfruit://pods`, …). The server is read-only.
...
3. **Point the agent at the MCP server.** The Agents SDK supports `MCPServerStdio`; wiring `dragonfruit_mock_server.py` into `get_producer_agent()` would let the agent ground its answers in pipeline/pods/wish-list context and would validate the MCP-first integration strategy before any external MCP server is adopted.

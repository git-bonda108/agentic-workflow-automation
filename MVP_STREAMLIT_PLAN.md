# MVP Streamlit Plan — the media agency AI & Automation Demo
...
- **Source:** Apply palette from **Winning_Proposal_Document_f715f6b4.pdf** (user-attached). If that PDF uses different hex values than our current the media agency set, we will:
  ...
  - Current the media agency palette in app: `--df-accent: #E85D75`, `--df-bg: #FAFAFA`, `--df-surface: #FFFFFF`, `--df-text: #1a1a1a`, `--df-text-muted: #64748B`, Plus Jakarta Sans.
...
| **By Role** | **New.** One tab per role: Producer, Production Coordinator, Creative Director, Post-Production Manager, Editor. Each tab: (1) role name + dream allocation one-liner, (2) **Tools this role uses** (from wish list + the agency-REFERENCE), (3) input/output per tool (from Wish List DOCX), (4) mock demo buttons that run with mock data and show result. |
...
## 3. Roles and tools (from the agency-REFERENCE)
...
## 4. Input/output per tool (from the agency-REFERENCE Wish List)
...
- **MCP (what we need):**
  - **Mock context:** **the media agency Mock Data MCP** (`mcp_servers/agency_mock_server.py`) — exposes `mock_data/*.json` as tools/resources. Used so agents (when running with SDK) can read pipeline, allocations, pods, wish list. For the Streamlit UI we don’t run MCP; we just load the same JSON. So for “mock demo” we **don’t need** to run the MCP server in the Streamlit process; we only need it when running the Producer (or other) agent with the SDK and want the agent to query mock data via MCP.
...
- **No hallucination:** All copy for “input/output” and “dream allocation” comes from the agency-REFERENCE (Wish List DOCX, Dream Allocations XLSX). We can add `mock_data/role_tools.json` that maps role → list of tool IDs + one-line dream text.
...
|-------|--------|--------------|
| **Batch 1 — Data & config** | Input/output and role→tools mapping from the agency-REFERENCE; color from Winning PDF if needed. | (1) Extend `mock_data/wishlist_tools.json` or add `mock_data/tool_io.json` with input/output per tool. (2) Add `mock_data/role_tools.json`: role → tools[] + dream_one_liner. (3) If Winning PDF colors differ, add to `COLOR_PALETTE.md` and a small `theme.py` or CSS snippet. |
...
| **Batch 3 — Run Demo polish** | Align Run Demo tabs with the agency-REFERENCE input/output; add dream allocation one-liner on Overview; ensure every tool tab shows Input / Output. | (1) Overview: “Dream allocation” sentence per role. (2) Each tool tab: Input (bullet list), Output (bullet list or chart/table). (3) Reuse existing mock runs; no new backends. |
...
- **Use cases = 16 tools** (8 high + 8 medium). Each has a **Run Demo** tab and appears under **By Role** for the role(s) that use it. We don’t need 16 separate backend services; we need:
  - **One** mock path per tool: when user clicks “Run” we either (a) call existing mock (e.g. CD-1 retention curve, ED-2 QC table, ED-3 assignees, PR-4 proposed task) or (b) show a stub “Output: [from the agency-REFERENCE]” with sample data.
  - **Shared data:** `mock_data/` + optional the media agency Mock Data MCP when running agents.
...
| Item | Approach |
|------|----------|
| **Color scheme** | From Winning_Proposal_Document PDF (or keep current the media agency palette). |
...
| **Tools per role** | From wish list + `role_tools.json`; input/output from the agency-REFERENCE. |
